...
func _initialize_compute():
	...
	# Create our shader
	var shader_file = load("res://assets/datamosh/datamosh.glsl")
	...
	shader_file = load("res://assets/datamosh/loop_frame.glsl")
	...
	if rd and p_effect_callback_type == CompositorEffect.EFFECT_CALLBACK_TYPE_POST_TRANSPARENT:
		...
		if render_scene_buffers:
			...
			for view in range(view_count):
				...
				push_constant.push_back(int(GlobalVariables.datamosh*100.0))
				
				if not GlobalVariables.refresh_frame:
